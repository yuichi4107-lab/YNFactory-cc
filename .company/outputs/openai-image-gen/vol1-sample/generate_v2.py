"""Generate one manga panel via gpt-image-2 for vol1-sample comparison (2026-04-22)."""
import os
import sys
import json
import base64
...
OUTPUT_DIR = os.path.join(
    PROJECT_ROOT,
    ".company", "outputs", "openai-image-gen", "vol1-sample", "v2",
)
...
print(f"Page {PAGE} ({entry['template']}) model={MODEL} size={SIZE} quality={QUALITY}: {len(ref_paths)} refs → {[os.path.basename(r) for r in ref_paths]}")
