...
EPUB_PATH = KDP_DIR / "ソマチッドとは何か 第1巻.epub"

TITLE = "ソマチッドとは何か 第1巻"
SUBTITLE = "ソマチッドという言葉の入口"
AUTHOR = "ソマチッド研究所"
WIDTH = 1024
HEIGHT = 1536
    ...
    with PANELS_CSV.open("r", encoding="utf-8-sig", newline="") as f:
        ...
        for row in reader:
            ...
            if template == "テキストページ":
                ...
                image = PAGES_DIR / f"page_{num:03d}.png"
                if not image.exists():
...
def build():
    ...
    with zipfile.ZipFile(EPUB_PATH, "w") as epub:
        ...
        epub.write(COVER_PNG, "OEBPS/images/cover.png", compress_type=zipfile.ZIP_DEFLATED)
        epub.write(COVER_JPG, "OEBPS/images/cover.jpg", compress_type=zipfile.ZIP_DEFLATED)
        epub.writestr("OEBPS/text/cover.xhtml", image_xhtml("cover", "cover.png", "表紙"), compress_type=zipfile.ZIP_DEFLATED)
        for page in pages:
            pid = f"page_{page['num']:03d}"
            if page["kind"] == "image":
                epub.write(page["image"], f"OEBPS/images/{pid}.png", compress_type=zipfile.ZIP_DEFLATED)
                xhtml = image_xhtml(pid, f"{pid}.png", f"ページ {page['num']}")
            else:
