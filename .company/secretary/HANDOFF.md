---
last_session_summary_v2026_06_20_ig_reels_fix: "【shorts-factory Instagram Reels 投稿の再試行・エラーハンドリング修正セッション（Mac）】shorts-factory の Instagram Reels 自動投稿で、ヘルパー(scripts/post_to_meta.py)がJSONを返さない／パース失敗した際に原因が不明瞭だった問題を修正。(1) shorts-factory/src/platforms/poster.py: post_instagram に _tail() ヘルパーを追加し、stdoutが空のとき returncode と stderr 末尾を含む明示的な RuntimeError を送出、JSONパース失敗時も stdout/stderr 両方の末尾をメッセージに含めるよう改善（従来は out[-300:] のみ）。(2) scripts/post_to_meta.py: Reels アップロードの再試行ハンドリングを修正（+130/-53）。(3) tests/test_posting_core.py に16行のテスト追加。これらは commit b73188d 『Fix Instagram Reels upload retry handling』として既にコミット済み。本ハンドオフ時点の未コミット変更は inputs 自動取り込みデータ（zoom指数・2026-06-20 lifelog/conversations）のみで一括コミット。残=オーナーが実際の IG Reels 投稿で改善後のエラーメッセージ／再試行挙動を本番確認。lifelog由来の当日アクション(投資獲得・競馬予想0件不具合・クラウドポート登録)は todos に転記。"
...
last_device: "Mac（shorts-factory の Instagram Reels 投稿の再試行・エラーハンドリングを修正。poster.py の post_instagram に _tail() 追加で stdout空/JSONパース失敗時のエラーメッセージを詳細化、scripts/post_to_meta.py の Reels アップロード再試行を改修、tests 追加。commit b73188d 済。未コミット=inputs自動取込のみで一括commit。残=本番IG投稿で挙動確認）旧: SNS導線の受け皿=営業導線を整備。ai.yn-factory.com がCloudflare Pages公開済→CTAライブラリのLP URL統一(status active)、short-video-cta-receiver.md(SNS→LP→無料AI導入診断の運用メモ・UTM・A/B/C返信文)とフォーム改修仕様(法人リード判別14項目)を新規作成、ローカルLP文面をSNS導線向けに更新。ブロッカー=フォームのライブ更新はy-nakada@yn-factory.com編集権限が要(yuichi4107では不可)。残=フォーム反映/LP公開承認/各SNSプロフィールURL統一。詳細は last_session_summary_v2026_06_19_sns_cta_receiver。前回=last_session_summary_v2026_06_18_shorts_factory_multiplatform）"
...
### 000-Z. [IN PROGRESS 2026-06-18] shorts-factory 複数媒体・1日3回・初中中バランス拡張 — 実装/QC完了・オーナーのログイン待ち 🔥
...
> - **実システム既知欠陥3点**: ①conversationsテーブル未記録(gmail_sender.py未実装) ②ウェビナー/予約URLは.env無効→personalizer.py(`src/tracks/c_outbound`)のPROMPT_TEMPLATE直書き要 ③From表示は案C(gmail.com送信、Reply-To/署名info@yn-factory.com)。
> - **次アクション(オーナー手動・最重要)**: LAUNCH_CHECKLISTの本番送信GO（DRY_RUN=false→自分宛1通確認→DAILY_LIMIT 1→5→30…）＋士業パートナー勧誘着手＋Peatixウェビナー7/15公開。以後 毎朝`/sales-briefing`・毎週月曜`/weekly-sales-review`。
