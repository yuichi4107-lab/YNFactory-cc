...
last_device: "Mac"
last_session_summary: "shorts-factoryの9時生成はネタ帳不足ではなく、6区間台本の実写cue 0/3が8.8秒・9.6秒となり、12秒素材の実写区間上限5秒を超えたため安全停止した。実写cueを35文字以下にする生成指示・検証・フォールバックを揃えてruntimeへ配備し、targeted testとhealth=okを確認。手動生成、Telegram承認、SNS投稿は行っていない。"
next_action: "次の14時枠で35文字上限に収まる即発話の実写cueを含む候補が生成されることを確認する。候補はTelegram承認後にだけ投稿する既存方針を維持する。"
---
...
### shorts-factory — 6区間の実写cue尺ガードを復旧 (最終更新: 2026-08-20)

- **状態**: runtime healthは`ok=true`（queue=256、媒体欠損・台帳異常なし）。9時枠はネタ帳ではなく実写cueの音声尺超過で安全停止した。失敗台本のcue 0/3は実測8.8秒・9.6秒で、12秒素材の各実写区間上限5秒を超えていた。
...
- **実写セリフ尺**: cue 0/3の`tts_kana`を35文字以下に必須化し、長い台本は映像生成前に不合格として再生成へ戻す。生成プロンプト・検証・フォールバックを同時更新してruntimeへ配備済み。9時の停止候補は再投稿せず、次の14時枠で確認する。
