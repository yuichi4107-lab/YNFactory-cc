---
last_session_summary_v2026_08_02_left_liberal_cover_jpeg: "【電子書籍『日本の左派・リベラルは、なぜ自滅するのか』JPEG表紙納品版】サブタイトル入り現行PNG表紙を、1024×1536・RGB・品質95・4:4:4・最適化・プログレッシブJPEGへ変換し、KDP出版用/cover.jpgを追加。PNG原本と両EPUBは変更なし。PSNR 41.74dB、独立QC100/100 PASS。"
last_session_summary_v2026_08_02_left_liberal_cover_subtitle: "【電子書籍『日本の左派・リベラルは、なぜ自滅するのか』表紙サブタイトル追加】ポップ版表紙に正式サブタイトル『最大の敵は保守ではない。自分たち自身である。』を濃紫帯・白字2行で追加。既存4文言、人物、4色配色、主タイトル最優先を保持。V2を保存しV3をcover.pngへ反映、EPUB再埋込、EPUBCheck全0、独立QC100/100 PASS。"
last_session_summary_v2026_08_02_left_liberal_cover_pop: "【電子書籍『日本の左派・リベラルは、なぜ自滅するのか』KDP表紙刷新】濃紺中心の旧表紙を保存し、黄色・シアン・コーラル・紫を使った日本マンガ＋雑誌広告風の高彩度表紙へ差し替え。必須4文言完全一致、100pxサムネ視認性合格、1024×1536 PNG。新表紙をEPUBへ再埋込しEPUBCheck 5.3.0 FATAL/ERROR/WARNING=0、独立視覚QC100/100 PASS。"
last_session_summary_v2026_08_02_left_liberal_public_source_renumber: "【電子書籍『日本の左派・リベラルは、なぜ自滅するのか』公開出典ID連番化】派生公開版の旧S003〜S070を新S001〜S068へ一対一変換し、本文引用256箇所（延べ288 ID）と出典一覧68件を整合。正本・sources・draftsは旧IDのまま保持。全78節の改ページを維持し、EPUBCheck 5.3.0はFATAL/ERROR/WARNING=0、独立QC100/100 PASS。再生成スクリプト2本も新仕様へ更新。"
...
last_session_summary_v2026_07_21_notion_input_sync: "【インプットデータNotion自動蓄積システム構築（Windows）】7/15音声メモの指示を実装完了。(1)要件定義=02_設定/requirements/notion-input-sync-2026-07-21.md（接続=Notion公式API+トークン/範囲=organized全部/単一DB+ビュー/1日1回、オーナー確認済み）。(2)実装=04_インプット/inputs/sync_notion.py: organized/{lifelogs,zoom,google-meet,external}/*.mdをNotion「インプットDB」へ同期。frontmatter→プロパティ(タイトル/日付/ソース/タグ/関連プロジェクト/優先度/TODO候補/input_id/元ファイル/取込日時)、本文→Notionブロック変換(2000字分割・100件バッチ)。冪等性=intake/state/notion_synced.jsonにsha256+page_id記録、変更時は旧ページarchive→再作成。レート制限0.35s+429/5xxリトライ、逐次state保存、logs/notion_sync_*.log。QC97点PASS。(3)認証=04_インプット/inputs/.env.notion(NOTION_TOKEN+NOTION_PARENT_PAGE_ID、.gitignore追加済み・git非コミット)。インテグレーション名=n8n-YNFactory(オーナー既存作成を流用)。Notion側はブラウザ操作で「YNFactory インプット」ページ(プライベート配下、id=3a3204bd-6a10-80f6-97b2-d3ae12a37f17)を新規作成しコネクト接続、DB自動作成(id=3a3204bd-6a10-817a-83f5-c2da72523c21)。(4)全92件バックフィル成功(failed=0)・再実行で全skipの冪等性確認・Task Scheduler「YNFactory Notion Sync」毎日07:30登録+手動トリガーexit 0確認。今後organized/に増えた分は毎朝自動でNotionへ蓄積される。コードはcommit af3d4e7でGitHub反映済み。注意: Mac側で同期する場合はlaunchd未設定(現状Windows専用)。"
...
- **2026-08-02追加修正**: 全78節を節頭改ページへ変更。出版用派生物から内部制作資料2件を完全除外し、残る旧S003〜S070を公開版S001〜S068へ一対一変換。本文引用256箇所（延べ288 ID）と出典一覧68件の対応不一致は0件。正本原稿・研究台帳は旧IDのまま変更なし。再生成スクリプト2本も同仕様へ更新し、EPUBCheck 5.3.0はFATAL/ERROR/WARNING=0、独立QC100/100 PASS。
- **2026-08-02表紙刷新**: 旧版を`KDP出版用/cover-v1-dark.png`へ保存し、黄色ベースの高彩度な日本マンガ＋雑誌広告風表紙を`cover.png`と`cover-v2-pop.png`へ反映。タイトル・フック・補助コピー・著者名は完全一致し、100pxサムネイルでも書名とフックを識別可能。新表紙をEPUBへ再埋込済み。EPUBCheck 0件、独立視覚QC100/100 PASS。
- **2026-08-02表紙サブタイトル追加**: 正式サブタイトル「最大の敵は保守ではない。自分たち自身である。」を主タイトル直下の濃紫帯へ白字2行で追加。`cover-v2-pop.png`を追加前版として保持し、`cover-v3-pop-subtitle.png`と`cover.png`を現行版とした。5文言完全一致、100pxで主タイトル・フック識別可能。EPUB再埋込、EPUBCheck全0、独立QC100/100 PASS。
- **2026-08-02 JPEG納品版追加**: `KDP出版用/cover.png`から、1024×1536・RGB・品質95・4:4:4・最適化・プログレッシブ形式の`KDP出版用/cover.jpg`を追加。PSNR 41.74dB。PNG原本と両EPUBのSHA-256は不変、独立QC100/100 PASS。
...
- **成果物**: `03_成果物/outputs/ebooks-manga/ynfactory-company-guide-10p/`（参照キャラ、ページ別プロンプト、表紙、`page_001.png`〜`page_010.png`、EPUB、PDF、進捗、品質レポート）。
...
- **やったこと**:
  ...
  - ローカルLP文面（`03_成果物/outputs/lp/ai-introduction-consult-publish/`、git管理外=Drive側のみ）をSNS導線向けに更新。
...
- **状態**: 要件定義2本(`02_設定/requirements/shorts-factory-autopost-improvements-2026-06-16.md`=approved/implemented、`shorts-factory-3x-multiplatform-midlevel-2026-06-16.md`=implemented_waiting_external_login)を実装→QC90点PASS。
- **やったこと**:
...
**プロジェクト場所**: `g:\マイドライブ\YNFactory-cc\03_成果物\outputs\ebooks-manga\chatgpt55-now-only-manga\`
...
**要件定義書**: `02_設定/requirements/chatgpt55-manga-redo-v2.md`（ADDENDUM v1+v2+v3 すべて追記済）
...
**関連ファイル**:
- 作業指示書: `02_設定/docs/engineering/engineering-docs/meta-sns-step6-claudeinchrome-instructions.md`（要更新: Console先行手順を追記）
...
**主な成果物**:
- `02_設定/requirements/ebook-to-manga-vol234-csv-redesign/REQUIREMENTS.md` + EXECUTION-phase{0,1-vol2,2-vol3,3-vol4}.md
...
- `02_設定/requirements/ebook-to-manga-codex-handoff/` — REQUIREMENTS.md + QA-phase1/2/3/step55-removal.md
...
**成果物**:
- `CLAUDE.md` — 整理済み（53行）
- `02_設定/docs/engineering/engineering-docs/ebook-to-manga-vol1-validation-requirements.md` — 検証要件定義書
- `03_成果物/outputs/ebooks-manga/manga-career-restart-validation/vol1/` — 検証出力一式
- `03_成果物/outputs/ebooks-manga/manga-career-restart-validation/vol1/pages/page_002_original_buggy.png` — バグ証拠画像
...
- `02_設定/docs/engineering/engineering-docs/ebook-to-manga-gpt-image-2-migration-requirements.md` — 要件定義書（2026-04-23更新版）
- `03_成果物/outputs/openai-image-gen/vol1-sample/comparison_v2.html` — NanoBanana2 vs gpt-image-2 vs chatgpt-image-latest 並列比較
- `03_成果物/outputs/openai-image-gen/vol1-sample/v2/p*.png` — 検証サンプル5枚（p002/p006/p012/p045 + p045_vertical + p045_overlay）
- `03_成果物/outputs/openai-image-gen/vol1-sample/generate_v2.py` — gpt-image-2 の最小実装例
...
- `02_設定/docs/engineering/engineering-docs/ebook-to-manga-hybrid-qc-requirements.md` — 要件定義書
...
>   - 要件定義: `02_設定/requirements/sales-system-2026-06/REQUIREMENTS.md`
...
- **設計書**: `02_設定/docs/engineering/engineering-docs/sales-ops-design.md`
...
- **新統合ルート**: `03_成果物/outputs/note-articles/`
...
- **成果物**:
  - `03_成果物/outputs/instagram-stories/batch-2026-04-15/prompts.md` (608行 / 53KB, 30本)
  - `03_成果物/outputs/instagram-stories/batch-2026-04-15/concepts.md` (30本テーマ一覧+重複チェック)
  - `03_成果物/outputs/instagram-stories/batch-2026-04-15/post_01〜30_*.png` (30枚完了)
  - `03_成果物/outputs/instagram-stories/batch-2026-04-15/generate_batch2.py` / `generate_log.txt`
...
- **最新セッション成果（2026-04-15）**:
  - 要件定義書承認（AP1完了）: `02_設定/docs/engineering/engineering-docs/jp-daytrade-v1-requirements.md`
...
- **要件定義書**: `02_設定/docs/engineering/engineering-docs/fx-phase1-requirements.md`
...
- **2026-04-05の作業**:
  ...
  - プロフィール文: `03_成果物/outputs/ai-side-business/profile-texts.md`
  - 更新指示書: `03_成果物/outputs/ai-side-business/profile-update-guide.md`
...
- **完成済み制作物** (`03_成果物/outputs/exhibition-booth/final/`):
...
### 8. 電子書籍・マンガ KDP出版
- **初回設定UI全ジャンル化（2026-08-01完了）**: `01_コード/scripts/company/ebook_setup_ui.py` を健康分野固定から汎用Phase 0へ改修。6問24選択肢、サーバー側入力検証、HTML/JS注入対策を追加し、`theme-to-ebook` / `ebook-from-theme` の両modeで既存保存形式を維持。テスト12件PASS、QC96/100。次回から任意テーマでそのまま利用可能
...
- **プレビューチェック中: 「出産でキャリアを失った元事務職ママが、AIで初めて稼ぐまで」（前日譚マンガ・4巻分冊）**
  - 原稿パス: `03_成果物/outputs/ebooks/manga-career-restart/manuscript/`
  - マンガ出力パス: `03_成果物/outputs/ebooks-manga/manga-career-restart/`
  - **4巻分冊構成（2026-04-08決定）**:
...
- **設計資産（API自動投稿用、将来使用）**:
  ...
  - 指示書: `02_設定/docs/engineering/engineering-docs/meta-api-setup-chrome-instructions.md`
...
- **要件定義書**: `02_設定/docs/engineering/engineering-docs/device-workflow-migration-requirements.md`（v2改訂済、2026-04-14）
...
- **note定期投稿 第2弾（4/13〜4/27、月水金12:00、7本）**: **2026-04-12に再作成完了**
  - 記事本文: `03_成果物/outputs/tech-articles/note-series/` に7本（執筆→QC91点合格）
  ...
  - カバー画像7枚: `03_成果物/outputs/note-series-covers/` に保存（NanoBanana2生成、16:9）
  - 本文画像14枚: `03_成果物/outputs/note-series-bodies/` に保存（各記事A:問題系+B:解決系）
