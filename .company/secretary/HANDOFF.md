...
last_device: "Mac"
last_session_summary: "shorts-factoryの停止を復旧。04:12に12秒の実写素材4本を全件検証して登録済みで、未使用在庫は8本。停止原因は、6区間フォールバック台本の長いテーマ名がタイトル上限32文字を超える不具合と、承認ボットのPIDロックが別プロセスを誤認して起動できない不具合だった。タイトルを短縮し、PIDの実コマンド照合と世代別ロック解除へ修正、77テストを通してMac runtimeへ配備。投稿・再投稿・承認は実行していない。"
next_action: "shorts-factoryの次の19時枠で新規候補が生成され、Telegramへ承認プレビューが届くことを確認する。承認後にだけ投稿する既存方針は維持する。"
---
...
- **状態**: runtime healthは`ok=true`（queue=255、媒体欠損・台帳異常なし）。04:12に12秒の`split_12s_v1`実写素材4本を全件検証・登録し、未使用在庫は8本。6区間フォールバック台本の長題名による上限違反を修正し、テスト77件を通過。
