...
- **状態**: 原稿完成。全16部・固定78節・本文相当125,794字。最終QC 97/100 PASS。
- **成果物**: `.company/projects/日本の左派リベラルはなぜ自滅するのか/`（要件、論証設計、70件の統合出典台帳、節別根拠表、章別16稿、統合原稿、品質レポート）。
- **品質**: 工程1調査95点、工程2論証設計97点、全章95〜98点、工程4最終97点。固定タイトル差分・台帳外出典・完全重複・禁止表現は0件。
- **留意点**: S040外務省URLは自動取得403（404ではない）。出典括弧は全角・半角が混在。公開直前に2026年の政党・選挙資料、係争・処分状況、S040到達性を再確認する。
- **未実施**: 表紙、挿絵、EPUB、KDPメタデータ、出版申請、公開・販売。
- **次のアクション**: オーナーが出版パッケージ化を希望した場合のみ、時点再確認→表記統一→EPUB/KDPメタデータ・表紙制作へ進む。
