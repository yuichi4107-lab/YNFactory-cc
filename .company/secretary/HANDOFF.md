...
last_device: "Mac"
last_session_summary: "shorts-factoryの停止を復旧。04:12に12秒の実写素材4本を全件検証して登録済みで、未使用在庫は8本。停止原因は、6区間フォールバック台本の長いテーマ名がタイトル上限32文字を超える不具合と、承認ボットのPIDロックが別プロセスを誤認して起動できない不具合だった。タイトルを短縮し、PIDの実コマンド照合と世代別ロック解除へ修正してMac runtimeへ配備。本番実行環境で6区間フォールバックの検証通過、health=ok、承認ボット稼働を確認。投稿・再投稿・承認は実行していない。"
