...
- **Meta（FB/IG）投稿トークンは無期限（2026-08-16 実測）**: `.company/engineering/sns-credentials/step6-tokens-2026-06-09.txt` の `USER_ACCESS_TOKEN_LONG` と `PAGE_ACCESS_TOKEN` は debug_token で `is_valid=true` / `expires_at=0`（期限なし）。`pages_manage_posts` `instagram_content_publish` を含む期待権限は**すべて付与済み**（2026-06-09時点で `MISSING_EXPECTED_PERMISSIONS` は空）。FBページ「YN Factory 出版プロデュース」(`1015019845037766`) と IG `@nakada_yuichi` (`17841477801881765`) へ読み取り疎通も確認済み。**「pages_manage_posts が Graph API Explorer に出ない」は 2026-05 時点の古い情報**。再検証は `python -c` で debug_token を叩けば足りる。なお `/{page-id}/feed` の**読み取り**だけは `Page Public Content Access` 未申請で403になるが、投稿には影響しない
- **Threads だけは別フロー（2026-08-16 時点）**: Threads は Graph API の権限候補に出ない。Threads API（`graph.threads.net`）の独立OAuthが必要でトークン未発行。FB/IGとは別案件として扱う
