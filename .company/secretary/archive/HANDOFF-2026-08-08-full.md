...
- **冒頭スライド多様化（2026-08-04）**: 生成失敗時の代替台本で `card_keywords[0]` が毎回`前提整理`に固定され、同じ動画に見える主因になっていた。通常版・Seedance版のプロンプトと実行時ガードを更新し、`前提整理`系を拒否、直近5本の同一先頭見出しを拒否、12候補（問いかけ・結論・失敗回避・比較・手順予告・具体例・チェック）を順に試すようにした。対象テスト4/4 PASS、runtime appへ同期済、QA 92/100 PASS。
- **未投稿ストック監査（2026-08-04）**: `ready_for_review` 7本は全件queue媒体状態`pending`かつposting ledgerなしで、投稿済みなし。`2026-06-29_090739_090739` は先頭`前提整理`のため再生成候補、`2026-07-15_092741_chatgpt-try2` と `2026-07-16_092121_chatgpt` は自己紹介テーマと導入が近く重複要確認。queue・動画・Telegram・投稿状態は変更していない。再生成・skip等の状態変更はオーナー個別承認後に行う。
- **Topview試作・再生成（2026-08-04〜05）**: Canvasプロジェクト「2026-08-04 12:26」に、提案書を持つ45歳前後の日本人ビジネスパーソンの実写広告風・縦9:16・10.054秒の冒頭ワンカットを1本生成。参照動画付きの初回はSeedance 2.0の参照動画15.2秒上限で失敗し、画面表示どおりクレジット返金。参照なしの再試行は成功。初回の4タスクはVideo 4/5しかCanvas反映を確認できず不採用候補とした。ユーザーの追加生成承認後、Video 3を人物参照にC1〜C4を再設計し、Video 6・Video 8・Video 9・Video 10（各10秒）の完了とCanvas反映を確認。開始0.0秒即発話、白字幕、人物/衣装/会議室固定、左45度→右後方肩越し→左プロフィール→高め斜め俯瞰の画角差、黒ペンを取る→書く→資料を中央へ寄せる→ペンを置くの動作接続を指定済み。タイムライン追加はUI操作の反応が不安定で接続状態未確定、映像・音声・字幕の実再生確認も未完了。書出し・ダウンロード・投稿はしていない。SNS投稿、Telegram送信、shorts-factory queue・ストックは未変更。
- **混在試作プレビュー（2026-08-05）**: 方針を「Topview=人物実写のみ／従来方式=日本語スライドのみ／外部編集=統合」に変更。ユーザー承認済みの書出し・合成範囲で、Topviewから取得できた実写2素材を、無字幕の desk full／talking upper／desk close／reaction close の4枠に再編集して日本語スライド4本と交互に合成した。完成=`.company/outputs/shorts-factory/hybrid-trial-2026-08-05/hybrid_trial_preview.mp4`（25.5秒、1080×1920、H264/AAC）。ナレーションは0.0秒開始、実写の元音声・BGMは未使用、焼き込み字幕は回避。フック→反論抽出→回答準備→行動喚起で「前提整理」型の反復を避け、独立QC88/100 PASS。元の実写は2本なので、量産前に4本以上の無字幕実写素材を確保し、反復感を下げる。SNS投稿・Telegram送信・queue/ストック変更なし。
- **男性ナレーション版（2026-08-05）**: ユーザー要望により、ナレーションを`VOICEVOX:青山龍星（ノーマル）`（speaker ID 13）の落ち着いた40代男性らしい声へ差し替え。映像は変えず、完成=`.company/outputs/shorts-factory/hybrid-trial-2026-08-05/hybrid_trial_preview_male_voice.mp4`。先頭0.0秒から発話、25.5秒・1080×1920・H264/AAC、実写の元音声とBGMは未使用。独立QC94/100 PASS。SNS投稿・Telegram送信・queue/ストック変更なし。
- **今後の男性ナレーション統一（2026-08-05）**: 今後の通常スライド版・実写（Seedance）版とも`VOICEVOX:青山龍星`（speaker ID 13、落ち着いた40代男性らしい声）を既定に統一。`shorts-factory/src/config.py`、`config.yaml.example`、README、話者回帰テストを更新し、runtime `~/shorts-factory/config.yaml`の話者・クレジットもID 13へ更新、コードを`~/shorts-factory/app`へ反映済み。runtime実効値・health=`ok:true`、queue=231状態不変を確認し、独立QC89/100 PASS。SNS投稿・Telegram送信・既存動画再生成なし。`test_voice_defaults.py`はPASS。`test_video_bg_gen.py`の既存フォールバック/読み整合の不一致は今回の話者変更とは無関係の別タスクとして残す。
- **runtime反映・在庫補充（2026-08-08 Mac）**: `bash 05_プロジェクト/shorts-factory/scripts/deploy.sh`（`install`なし）を実行し、8/8の4点修正をruntime `~/shorts-factory/app` へ反映。①安全停止のTelegram通知（`pipeline.py` に `notify_safe_stop`）、②在庫低下の事前警告（`pipeline.py:1318`、`low_stock_warn_clips` 既定6）、③`register_files` 追記マージ（`topview_inventory.py:135`）、④README・config例。launchd再登録は不要（生成ジョブは毎回起動し直し、`approval_bot.py` は変更対象外）。
- **Topview在庫補充（2026-08-08 Mac）**: Canvas `654e5964324b4707b12c890e13249039` に未書き出しの生成済みクリップが Video 13/14/15 だけでなく **16/17/18 まで計6本**あることを確認し、6本すべてを書き出して登録。**新規生成なし＝クレジット消費ゼロ（残高287.28のまま）**。全6本 720×1280（9:16）・15.07秒・md5相異、各クリップのプロンプトに `No on-screen text, no subtitles, no music, no generated audio` を確認、18フレーム（各3点）の目視で焼き込み文字なし・人物/服装の一貫性を確認。登録結果は**有効8本／未使用8本（動画4本ぶん）**で `validate_inventory` と `select_live_clips(2)` が通過し、安全停止は解消。`Video_12` は4媒体投稿済みのため意図的に未登録（`registered? False` を確認）、`Video_8/9` の使用履歴も追記マージで保持。`~/shorts-factory/topview_assets/pending_topview_assets.json` を `exported` へ更新済み。
- **解像度の混在に注意**: 新規6本は720p、既存 `Video_8/9` は1080×1920。最終レンダリングで拡大される可能性があるため、Topview枠の仕上がりは一度目視確認する。Topviewの「高画質化」はクレジット消費のため未実行。
- **次回補充の余地**: Canvasには未書き出しの Video 3〜7・10・11 も残っている。8本＝動画4本ぶん（3枠/日で約1.3日分）なので、次回はまずここから追加補充できるか確認する。
- **Topview書き出し手順（次回用）**: Canvasでクリップを選択 → ツールバーの ⬇ → 数十秒待って `~/Downloads` を確認 → `~/shorts-factory/topview_assets/` へ移動 → `cd ~/shorts-factory/app && ~/shorts-factory/.venv/bin/python scripts/register_topview_assets.py <file...>`。保存の反映に時間差があるので、直後にファイルが無くてもブロックと即断しない。
- **Git反映**: 2026-08-08セッションでcommit/push実施（下記「ダウンロード事前承認」ぶんを含む）。
