...
CRITERIA = [
    ...
    {'label': '募集総額', 'short': '価格帯', 'col': 'total_man', 'lo': 2500, 'hi': 3999,
     'cond': '2500〜3999万円'},
...
def main():
    ...
    L.append(['', '', ''])
    L.append(['◆スコア基準(各1点・計3点)', '条件', '根拠(中央勝ち上がり/回収≥1)'])
    ...
    L.append(['2. 募集総額', '2500〜3999万円',
              f'{pb["win_jra"]}%・{pb["ret1"]}%（{pb["n"]}頭）。'
              f'2500万未満は{lo_b["win_jra"]}%・{lo_b["ret1"]}%（{lo_b["n"]}頭）と落ち、'
              f'4000万以上は{hi_b["win_jra"]}%・{hi_b["ret1"]}%（{hi_b["n"]}頭）で走るが回収が伸びない'])
    w_hi = band('weight', 420, 9999)
    w_lo = band('weight', 0, 419)
    L.append(['3. 馬体重', '募集時420kg以上',
    ...
    L.append([f'新スコア(3点)', f'中央勝ち上がり{auc_new["win"]} / 回収≥1 {auc_new["ret"]}',
              '年度別 ' + ' '.join(f'{y}:{v["new"]}' for y, v in auc_year.items())])
    L.append([f'既存6基準', f'中央勝ち上がり{auc_old["win"]} / 回収≥1 {auc_old["ret"]}',
