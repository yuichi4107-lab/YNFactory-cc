...
WINRATE = {3: "67%", 2: "54%", 1: "45%", 0: "21%"}
# スコア別の「賞金が募集総額を超えた馬」の割合
RETRATE = {3: "37%", 2: "16%", 1: "16%", 0: "2%"}
...
def verdict(score, priority, p, sex, weight):
    ...
    走る側はスコア別の実績（3点=中央勝ち上がり67%・回収≥1が37% /
    2点=54%・16% / 1点=45%・16% / 0点=21%・2%）、
...
READING = [
    ...
    ["   ・走るか … 過去5年（2020〜2024年度募集）467頭のデビュー後成績から導いた3点満点スコア"],
    ...
    [""],
    ["■ スコア（0〜3点。各1点）"],
    ["  牡馬 / 募集総額2500〜3999万円 / 募集時馬体重420kg以上"],
    ["  過去5年の実績（中央400口444頭。勝ち上がりは中央で1勝以上）"],
    ["    3点(52頭) 中央勝ち上がり67%・回収≥1が37%・回収中央値0.7"],
    ["    2点(202頭) 54%・16%・0.24"],
    ["    1点(147頭) 45%・16%・0.22"],
    ["    0点(43頭) 21%・2%・0.04"],
    ["  2026/8に3年→5年へ広げて基準を作り直した。3〜4月生・ノーザンF生産・母8〜11歳は"],
    ["  5年では効かず（年度ダミーつきで有意でない）落とした。詳細は「検討基準」シート"],
    [""],
    ["■ 枠が埋まる確率"],
    ...
    [""],
    ["■ 判断区分"],
    ["  S 最有力    … スコア3 かつ 優先枠が埋まる確率50%未満（または母馬優先対象外）"],
    ["  A 有力      … スコア2 で競争が緩い、またはスコア3だが人気で競争が厳しい"],
    ["  B 検討      … スコア2 かつ 優先枠が埋まる確率50%以上"],
    ["  C 見送り寄り … スコア1"],
    ["  D 回避      … スコア0（過去5年43頭で中央勝ち上がり21%・回収が募集額を超えたのは1頭）"],
    [""],
    ["■ 回避条件"],
    ...
    ["  ・当たり具合はAUC0.61（勝ち上がり）／0.62（回収）。でたらめが0.5なので、効きは穏やか。"],
    ["  ・2023年度募集世代は現4歳、2024年度募集世代は現3歳。成績は今後上振れし得る。"],
    ["  ・2026年度は募集価格が高く（中央値5000万）、価格基準に当てはまるのは94頭中20頭。"],
