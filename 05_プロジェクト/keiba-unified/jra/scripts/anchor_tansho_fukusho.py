"""Longshot Wide の軸馬で単勝・複勝を買ったらどうなるか"""
...
VARIANTS = [
    # (label, bet_type, anchor_min_pop, anchor_min_prob)
    ("Win pop7 p0.25",    "単勝", 7, 0.25),
    ("Win pop7 p0.30",    "単勝", 7, 0.30),
    ("Place pop7 p0.25",  "複勝", 7, 0.25),
    ("Place pop7 p0.30",  "複勝", 7, 0.30),
    ("Win pop5 p0.25",    "単勝", 5, 0.25),
    ("Place pop5 p0.25",  "複勝", 5, 0.25),
    ("Win pop7 p0.25 odds<=50",  "単勝", 7, 0.25),  # max_odds flag
    ("Place pop7 p0.25 odds<=50","複勝", 7, 0.25),
    # Compare with the Wide strategy
    ("Wide3flow pop7 p0.25 part0.35", "ワイド", 7, 0.25),
]

print(f"{'Label':<38} {'Bets':>5} {'Hit%':>6} {'ROI':>7} {'Net':>12} {'AvgPay':>7}")
print("-" * 82)

for label, bet_type, a_pop, a_prob in VARIANTS:
    t0 = time.time()
    max_odds = 50 if "odds<=50" in label else 80
    ...
    for rid, rdf in test.groupby("race_id"):
        ...
        anchors = [h for h in horses
                   if h["pop"] >= a_pop and h["prob"] >= a_prob and h["odds"] <= max_odds]
        if not anchors:
            continue
        ...
        if bet_type == "ワイド":
            ...
            combo = str(anchor["num"])
            amt = max(100, round(bankroll * 0.005 / 100) * 100)
            amt = min(amt, bankroll * 0.02)
            b = Bet(bet_type=bet_type, combination=combo, amount=amt,
                    odds=anchor["odds"], expected_value=0, horse_numbers=[anchor["num"]])
            b.race_id = rid; b.race_date = rd
            b.is_hit = engine._check_hit(b, results_df)
            b.payout = engine._calculate_payout(b, payoffs_df) if b.is_hit else 0.0
            b.profit = b.payout - b.amount
