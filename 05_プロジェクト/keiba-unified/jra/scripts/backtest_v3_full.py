...
class PureModelUmarenStrategy:
    """馬連: モデル Top-2 馬の組み合わせに賭ける。実払戻は engine が payoffs から参照。"""

    def __init__(self, min_prob_each=0.35, min_joint_prob=0.15,
                 min_horses=10, bet_fraction=0.01):
        self.min_prob_each = min_prob_each
        self.min_joint_prob = min_joint_prob
        self.min_horses = min_horses
        self.bet_fraction = bet_fraction
    ...
    def generate_bets(self, race_df, probas, bankroll):
        ...
        for i, (idx, row) in enumerate(race_df.iterrows()):
            ...
            if prob < self.min_prob_each or odds <= 0:
                continue
            horses.append({"num": horse_num, "prob": prob, "odds": odds})
        ...
        # 2頭とも Top-2 に入る同時確率（近似：各馬 Top-3 確率の積 × 2）
        joint_prob = h1["prob"] * h2["prob"] * 2.0
        if joint_prob < self.min_joint_prob:
            return []

        nums = sorted([h1["num"], h2["num"]])
        combo_str = f"{nums[0]}-{nums[1]}"
        umaren_odds_est = max(2.0, (h1["odds"] * h2["odds"]) * 0.5)
        ...
        return [Bet(
            ...
            combination=combo_str,
            amount=amount,
            odds=umaren_odds_est,
            expected_value=joint_prob * umaren_odds_est,
            horse_numbers=nums,
        )]


class PureModelSanrenpukuStrategy:
    """三連複: モデル Top-3 馬の単一組み合わせ。実払戻は engine が payoffs から参照。"""

    def __init__(self, min_prob_each=0.30, min_joint_prob=0.05,
                 min_horses=10, bet_fraction=0.01, mode="top3"):
        """mode: 'top3' = top-3 単一BOX, 'top4_box' = top-4 BOX (4C3=4点)"""
        self.min_prob_each = min_prob_each
        self.min_joint_prob = min_joint_prob
        self.min_horses = min_horses
        self.bet_fraction = bet_fraction
    ...
    def generate_bets(self, race_df, probas, bankroll):
        ...
        if self.mode == "top3":
            ...
            joint_prob = h1["prob"] * h2["prob"] * h3["prob"] * 6.0
            if joint_prob < self.min_joint_prob:
                return []

            nums = sorted([h1["num"], h2["num"], h3["num"]])
            combo_str = f"{nums[0]}-{nums[1]}-{nums[2]}"
            odds_est = max(5.0, (h1["odds"] * h2["odds"] * h3["odds"]) * 0.2)
            ...
            return [Bet(
                ...
                amount=amount,
                odds=odds_est,
                expected_value=joint_prob * odds_est,
                horse_numbers=nums,
            )]

        if self.mode == "top4_box" and len(horses) >= 4:
            top4 = horses[:4]
            # 4C3 = 4 combos
            combos_idx = [(0, 1, 2), (0, 1, 3), (0, 2, 3), (1, 2, 3)]
            joint_prob_global = (top4[0]["prob"] * top4[1]["prob"]
                                 * top4[2]["prob"] * 6.0)
            if joint_prob_global < self.min_joint_prob:
                return []

            bets = []
            per_combo_fraction = self.bet_fraction / 4.0
            for (a, b, c) in combos_idx:
                ha, hb, hc = top4[a], top4[b], top4[c]
                nums = sorted([ha["num"], hb["num"], hc["num"]])
                combo_str = f"{nums[0]}-{nums[1]}-{nums[2]}"
                odds_est = max(5.0, (ha["odds"] * hb["odds"] * hc["odds"]) * 0.2)
                joint = ha["prob"] * hb["prob"] * hc["prob"] * 6.0

                amount = bankroll * per_combo_fraction
                amount = max(100, round(amount / 100) * 100)
                amount = min(amount, bankroll * 0.02)
...
def main():
    ...
    # 馬連戦略（モデル Top-2 同時確率ベース）
    for min_joint in [0.10, 0.15, 0.20]:
        name = f"Umaren joint>={min_joint:.2f}"
        strategies[name] = PureModelUmarenStrategy(
            min_prob_each=0.30, min_joint_prob=min_joint,
            min_horses=10, bet_fraction=0.01,
        )

    # 三連複戦略（Top-3 単一BOX）
    for min_joint in [0.03, 0.05, 0.08]:
        name = f"Sanren joint>={min_joint:.2f}"
        strategies[name] = PureModelSanrenpukuStrategy(
            min_prob_each=0.25, min_joint_prob=min_joint,
            min_horses=10, bet_fraction=0.01, mode="top3",
        )

    # 三連複 Top-4 BOX 戦略
    strategies["Sanren top4_box"] = PureModelSanrenpukuStrategy(
        min_prob_each=0.25, min_joint_prob=0.05,
        min_horses=10, bet_fraction=0.01, mode="top4_box",
    )
    ...
    mkt_strategies = {
        ...
        "Umaren+Mkt joint>=0.15": PureModelUmarenStrategy(
            min_prob_each=0.30, min_joint_prob=0.15,
            min_horses=10, bet_fraction=0.01,
        ),
        "Sanren+Mkt joint>=0.05": PureModelSanrenpukuStrategy(
            min_prob_each=0.25, min_joint_prob=0.05,
            min_horses=10, bet_fraction=0.01, mode="top3",
        ),
        "Sanren+Mkt top4_box": PureModelSanrenpukuStrategy(
            min_prob_each=0.25, min_joint_prob=0.05,
            min_horses=10, bet_fraction=0.01, mode="top4_box",
        ),
        "Wide+Mkt edge>=0.05": PureModelWideStrategy(
            min_prob_each=0.30, min_edge_each=0.05,
            min_horses=10, max_combos=3, bet_fraction=0.01,
        ),
    }
    ...
    comparison_path = "data/reports/v3_full_comparison.html"
    ...
    if all_metrics[best_name]["total_bets"] > 0:
        ...
        best_path = f"data/reports/v3_full_best_{safe_name}.html"
