...
# -*- coding: utf-8 -*-
"""
競馬予想 v3 純粋実力モデル（複勝予測=3着内）
オッズ・人気を除外した特徴量で3着内確率を予測
"""
...
def train_model(conn, train_start="2022-01-01", train_end="2024-12-31"):
    ...
    # 2. ラベルを3着内=1に変換
    df["label"] = (df["finish_position"] <= 3).astype(int)
    ...
    pos_rate = y.mean()
    print(f"\n学習データ: {len(X)}行, 正例率（複勝率）: {pos_rate:.3f}")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage:")
        print("  python3 model_v3_place.py train                        # 学習")
        print("  python3 model_v3_place.py train 2022-01-01 2024-12-31  # 期間指定")
        print("  python3 model_v3_place.py predict 202503020911          # 1レース予測")
        sys.exit(1)
