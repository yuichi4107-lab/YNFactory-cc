...
# スコア別の勝ち上がり率（2020〜2022年募集276頭の実績。検討基準シートより）
WINRATE = {6: "100%", 5: "82%", 4: "67%", 3: "55%", 2: "55%", 1: "55%", 0: "55%"}
...
def verdict(score, priority, p, sex, weight):
    ...
    走る側はスコア別の勝ち上がり実績（6点100% / 5点82% / 4点67% / 3点以下55%）、
...
def avoid_flag(sex, weight):
    """検討基準シートの明示的な回避条件：メスの430kg未満。"""
    if sex and "メ" in str(sex) and weight not in (None, "") and float(weight) < 430:
        return "メス430kg未満（勝ち上がり42%・回収中央値0.09）"
    return ""
...
COLS = [
    ...
    ("スコア", HEAD_B, 7), ("勝ち上がり実績", HEAD_B, 11), ("スコア内訳", HEAD_B, 26),
...
def sheet_unified(wb, brows, fc, alist):
    ...
    for b in brows:
        ...
        row = [
            ...
            score, WINRATE.get(score or 0), b.get("スコア内訳"),
            "○" if priority else "",
    ...
    for r in range(2, ws.max_row + 1):
        for c in range(1, len(COLS) + 1):
            ...
            if c in (1, 5, 6, 10, 11, 12, 13, 16, 19, 20, 21, 22, 23, 24):
                cell.alignment = Alignment(horizontal="center")
        pc = ws.cell(r, 23)
        if pc.value is not None:
            pc.number_format = "0%"
        v = ws.cell(r, 26).value
        fill = {"S 最有力": F_S, "A 有力": F_A, "A 有力（人気）": F_A,
                "B 検討": F_B}.get(v)
        if fill:
            for c in (2, 16, 26):
                ws.cell(r, c).fill = fill
        if ws.cell(r, 27).value:
            ws.cell(r, 27).fill = F_AVOID
...
READING = [
    ...
    ["   ・走るか … 過去3年（2020〜2022年募集）276頭のデビュー後成績から導いた6点満点スコア"],
    ...
    [""],
    ["■ スコア（0〜6点。各1点）"],
    ["  牡 / 3〜4月生 / ノーザンF生産 / 母8〜11歳 / 総額2500万〜6000万未満 / 馬体重430kg以上"],
    ["  過去実績: 6点=勝ち上がり100%(12頭) 5点=82% 4点=67% 3点以下=55%"],
    ["  ※ 詳細と父系・厩舎の傾向は「検討基準」シート"],
    [""],
    ["■ 枠が埋まる確率"],
    ...
    [""],
    ["■ 判断区分"],
    ["  S 最有力    … スコア5〜6 かつ 優先枠が埋まる確率50%未満（または母馬優先対象外）"],
    ["  A 有力      … スコア4 で競争が緩い、またはスコア5〜6だが人気で競争が厳しい"],
    ["  B 検討      … スコア4 かつ 優先枠が埋まる確率50%以上"],
    ["  C 見送り寄り … スコア3以下"],
    [""],
    ["■ 回避条件"],
    ["  メスの430kg未満は過去57頭で勝ち上がり42%・回収中央値0.09と最も厳しい組み合わせ。"],
    ["  判断区分と独立に、別列で印を付けている。"],
    [""],
    ["■ 注意"],
    ["  ・スコアは母数276頭の粗い層別であって、個別の馬の能力予測ではない。"],
    ["  ・2022年募集世代はまだ現役が多く、成績は今後上振れし得る。"],
    ["  ・厩舎相性は各厩舎5〜12頭の小標本。参考程度に。"],
