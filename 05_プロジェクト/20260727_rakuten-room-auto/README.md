...
- **自動ネタ補充（既定）**: 残りネタ（未投稿・承認待ち・承認済）が閾値（既定5件）以下になったら、楽天ランキングから実在商品を取得し、紹介文付きで既定5件を「未投稿」として自動追加します。シート内の既存URLとは重複しません。ジャンル・件数は `config.yaml` の `replenish` で調整、`replenish.enabled: false` で無効化できます。
- **スコアリング選定（推奨）**: 楽天ウェブサービスの認証情報（下記）を `.env` に設定すると、補充が公式ランキングAPIベースに切り替わります。ランキング上位を上から採るのではなく、「順位 + 急上昇（前回実行比） + レビュー件数・評価 + 価格帯 + アフィリエイト料率」でスコアリングした上位から補充し、紹介文も商品ごとに文面パターンを変えて生成します（定型文の量産はROOMでいいねが付きにくいため）。急上昇検出のためランキング履歴を `~/rakuten-room-auto/data/ranking_snapshots.json` に保存します。認証情報が無い場合・API障害時は従来のランキングページ読み取りに自動フォールバックします。

  ```text
  # ~/rakuten-room-auto/.env に追記
