...
- **1.2 での変更**: 受注の品目情報を `sales_orders`(案件ヘッダ)と `sales_order_lines`(受注明細)の親子構造に分離した。初期版の運用は従来どおり**1受注=1明細**(アプリ層で制限)であり画面・帳票の見た目は変わらないが、将来の複数品目・分納・部分請求にテーブル構造の作り直しなしで対応できる。データ投入後の分離は移行コストが高いため実装前に確定させる
...
erDiagram
    ...
    purchase_orders {
        integer id PK
        integer sales_order_id FK
        integer sales_order_line_id FK "対象明細(任意)"
...
受注は `sales_orders`(案件ヘッダ: 管理No.・日付・客先)と `sales_order_lines`(受注明細: 品目・数量・単価)の親子構造とする。初期版は明細1行に制限して運用するため画面・帳票は現行のままだが、複数品目への拡張時にテーブルを作り直さずに済む。
...
- 品目情報(品名・型式図番・数量・単位・単価・備考)は `sales_order_lines`(2.4)へ持つ。本テーブルは**案件ヘッダ**であり品目を持たない
- 受注額(受注小計)は保存せず、明細の `quantity * unit_price` の合計を計算で得る(冗長保存による不整合を避ける)
- ステータスは**案件単位**で持つ。明細ごとに納品・請求の状態が分かれるのは分納・部分請求を導入する場合であり、これらはスコープ外(要件定義書7章)のため初期版では発生しない
- インデックス: `order_no`, `customer_id`, `order_date`, `status`

### 2.4 sales_order_lines(受注明細)

| 列 | 型 | 制約 | 説明 |
|----|----|------|------|
| id | INTEGER | PK AUTOINCREMENT | |
| sales_order_id | INTEGER | NOT NULL FK→sales_orders.id | 親の受注(案件) |
| line_no | INTEGER | NOT NULL DEFAULT 1 | 行番号(1始まり)。同一 sales_order_id 内で UNIQUE。表示・印字の並び順 |
| product_name | TEXT | NOT NULL | 品名(例: シャフト) |
| model_no | TEXT | | 型式・図番(例: A0010) |
...
- 小計は保存せず `quantity * unit_price` を計算で得る
- **初期版の制限**: 1受注につき明細は1行のみ。DB制約ではなく**アプリ層で制限**する(画面に行追加ボタンを出さない)。複数明細を許可する際はこの制限を外すだけでよく、テーブル定義・帳票の明細ループ・請求の紐付けは変更不要
- 親の受注が削除・取消された場合、明細は `ON DELETE CASCADE` で追従する
- インデックス: `sales_order_id`

### 2.5 purchase_orders(発注)

| 列 | 型 | 制約 | 説明 |
|----|----|------|------|
| id | INTEGER | PK AUTOINCREMENT | |
| sales_order_id | INTEGER | NOT NULL FK→sales_orders.id | 紐付く受注(案件) |
| sales_order_line_id | INTEGER | NULL FK→sales_order_lines.id | 対象の受注明細。どの品目に対する発注かを記録する。初期版は明細が1行のみのため自動でその行を設定する。案件全体に対する発注(品目に対応しない諸経費等)は NULL を許容 |
| supplier_id | INTEGER | NOT NULL FK→suppliers.id | |
| po_date | DATE | NOT NULL | 発注日 |
| supplier_due_date | DATE | NOT NULL | 発注納期 |
| product_name | TEXT | NOT NULL | 受注明細から引継ぎ(編集可) |
| model_no | TEXT | | |
...
- 品名・型式図番・数量・単位・備考の初期値は `sales_order_line_id` が指す受注明細から引き継ぐ(NULL の場合は引継ぎなし)
...
- 納品は**案件単位**で記録する(初期版は1受注=1明細=1納品)
- 分納対応時は UNIQUE を外し、`sales_order_line_id` と納品数量の列を追加して明細単位の分納に拡張する(将来拡張)
...
### 2.8 invoice_lines(請求明細)

| 列 | 型 | 制約 | 説明 |
|----|----|------|------|
| id | INTEGER | PK AUTOINCREMENT | |
| invoice_id | INTEGER | NOT NULL FK→invoices.id | |
| sales_order_line_id | INTEGER | NOT NULL FK→sales_order_lines.id | 請求対象の受注明細。**明細行単位で紐付く**(案件単位ではない)ため、1案件に複数明細がある場合は請求書に複数行として並ぶ |
| amount | INTEGER | NOT NULL | 確定時点の小計(数量×単価) |

- 制約: 有効な請求(status != 'cancelled')において同一 sales_order_line_id が重複しないことをアプリ層で保証(二重請求防止)
- 初期版は1受注=1明細のため、実質的に案件単位の紐付けと同じ動作になる

### 2.9 attachments(添付ファイル)【オプション・初期版では未作成】
...
### 2.10 users(ユーザー)【オプション・初期版では未作成】
...
| 項目 | ルール |
|------|--------|
| 明細小計 | 受注明細の 数量 × 単価(税抜)。例: 2 × 10,000 = 20,000 |
| 受注額 | 案件に属する受注明細の小計合計。初期版は明細1行のためその小計と一致する |
| 発注額 | 案件に紐付く発注小計の合計。例: 2 × 8,000 = 16,000 |
| 粗利 | 受注額 − 発注額。例: 20,000 − 16,000 = 4,000。**案件単位**で算出する(明細単位の粗利は、発注が明細に紐付かない場合があるため初期版では出さない) |
