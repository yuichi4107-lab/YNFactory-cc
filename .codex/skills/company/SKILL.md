...
name: company
description: >
  [廃止 2026-08-09] 仮想会社組織（秘書→CEO→各部署）の構築・運営スキル。
  この運用は2026-08-06に廃止され、要件定義 → 実行 → 品質チェックの3エージェントに一本化された。
  新規に使わない。日々のTODO・引き継ぎは `02_設定/docs/company-ops.md`、
  作業の進め方は `02_設定/docs/quality-loop.md` を見る。
  過去の運営記録の参照が必要なときだけ、`99_その他/company-records/` を読む。
---

# 仮想カンパニー

> **[廃止 2026-08-09] このスキルは使わない。**
>
> 「秘書→CEO→各部署」の組織の比喩による運用は2026-08-06に廃止された。
> 本スキルが前提とする `.company/CLAUDE.md`・`.company/secretary/CLAUDE.md` は
> `99_その他/company-records/` へ退避済みで、`pm/tickets/`・`ceo/decisions/`・
> `genspark/done/` も同様に移動している。**記載どおりのパスには存在しない。**
>
> | やりたいこと | 現行の参照先 |
> |---|---|
> | TODO・引き継ぎ | `02_設定/docs/company-ops.md` |
> | 作業の進め方 | `02_設定/docs/quality-loop.md` |
> | 承認の境界 | `02_設定/docs/approval-rules.md` |
> | 置き場のルール | `02_設定/docs/folder-structure.md` |
> | オーナー情報・対話スタイル | `02_設定/docs/owner-profile.md` |
>
> 以下は経緯を追うための参照専用。実行しないこと。
