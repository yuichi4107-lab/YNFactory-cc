...
def make_fake_drive(root: Path) -> None:
    ...
    touch(root / ".company" / "scripts" / "sync_drive_git.py")
    touch(root / ".company" / "requirements" / "req-a.md")
    touch(root / ".company" / "engineering" / "docs" / "design.md")
    touch(root / ".company" / "projects" / "案件A" / "note.md")
    touch(root / ".company" / "manus" / "archive" / "old.md")
...
class OrganizeDriveRootTest(unittest.TestCase):
    ...
    def test_company_dissolves_into_buckets_but_secretary_stays(self) -> None:
        organize(self.root)
        self.assertTrue((self.root / "01_コード" / "scripts" / "company" / "sync_drive_git.py").exists())
        self.assertTrue((self.root / "02_設定" / "requirements" / "req-a.md").exists())
        self.assertTrue((self.root / "02_設定" / "docs" / "engineering" / "engineering-docs" / "design.md").exists())
        records = self.root / "99_その他" / "company-records"
        self.assertTrue((records / "projects" / "案件A" / "note.md").exists())
        self.assertTrue((records / "manus" / "archive" / "old.md").exists())
        self.assertFalse((self.root / ".company" / "projects").exists())
        self.assertFalse((self.root / ".company" / "scripts").exists())
        # 引き継ぎ系はパスが外部に埋まっているので動かさない
        self.assertEqual(
            (self.root / ".company" / "secretary" / "HANDOFF.md").read_text(encoding="utf-8"),
            "handoff",
        )
