...
7. 画像生成が必要な場合は `openai-image-gen` スキルを使う。
...
現在の標準運用では予約投稿を使わない。ユーザーが明示的に「予約投稿に戻す」「指定日時で予約して」と依頼した場合のみ、この章を使う。即時投稿の依頼ではこの章を実行しない。
