...
スキル起動直後、リサーチや原稿作成を始める前に、以下の項目をまとめて提示してユーザーの回答を待つ。クリック式UIが使える場合はクリック式で提示する。使えない場合は、標準プランを先に示し、ユーザーが「この内容で開始」または変更したい項目だけを書けば済むMarkdown UIにする。最後に自由記述欄を付ける。
...
| カラー | フルカラー / 白黒 |
| キャラクター | 全部お任せ / 主人公だけ指定 / 全員指定 |
| マンガページ数 | 3ページ / 4ページ / 5ページ |
| マンガ配置 | 章末 / 章頭 |
| 本文文字数 | 15,000字 / 25,000字 / 35,000字 |

返信例:
この内容で開始

返信例:
本文文字数=35,000字、マンガ配置=章頭
...
- ユーザーに `1A、2B...` のような長いコード型回答を要求しない。
